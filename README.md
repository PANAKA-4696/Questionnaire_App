...
- [x] **今後の学習課題**: 今回はAndroid View Systemで開発しましたが、現在の主流であるJetpack ComposeでのUI構築スキルや、最新の技術、主流を習得するため、簡単なアプリ作成などを通して学習を進めます。  現在練習中のGitHubリポジトリ(https://github.com/PANAKA-4696/PracticeToDoApp)
  
他エンジニアからのFBもいただいたためそちらも記述していこうと思います。
- [ ] **DB操作のクラスファイル分け** :重複のある部分をファイルとしてクラス分けすることでクラス呼び出しで記述をもっとコンパクトにする必要性がある。
- [ ] **バックアップファイルだけがなくなる可能性のある処理の改善** :backupDatabaseToDriveの際に削除してから新しいファイルにすることのデメリットとして、新しいファイルを作成した際にバックアップファイルが削除されて新しいファイルも作れないという可能性があるその部分の修正。  
- [ ] **DBに関するライブラリの使用(考慮してもよい)** :もっと効率や便利なものを作成するにはGitHubなどの★が多いライブラリを使用することも意識したほうが良い。実務では便利なものをより使う日ちようがあるため実務に知被けるという意味でもライブラリについて知る必要があるとのことです。  
- [ ] **アプリ内でもログインの実装をする必要がある** :アプリとしての活用だけでなくリスクの意識を持つ必要がある。端末をなくしてしまった際に顧客情報が他人に見られてしまう危険性があるためアプリ自体にログイン機能を設ケル必要性。  
- [ ] **通信傍受の可能性を考慮し、データの暗号化** :基本アプリ内での完結ではあるが、もしセキュリティに対しての考慮があるのであれば、通信傍受の可能性を考慮して、暗号化する必要性を考えたほうがいい。  
